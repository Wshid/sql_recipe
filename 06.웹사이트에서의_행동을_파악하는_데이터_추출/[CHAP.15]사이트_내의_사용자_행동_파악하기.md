...
## 9. 사용자 행동 전체를 시각화하기
- **각각의 리포트**를 기반으로,
  - 현재 상황을 분석하고 대책을 세우는 것은 당연하나,
  - 이 데이터는 어디까지나 **단편적**이며, 굉장히 **국소적**인 정보
- 각 항목에서의 내용 및 **접근 분석 도구**의 출력 리포트는
  - 전체적인 느낌을 파악하기 어려운 면이 있음
  - 마치 `점`과 같은 정보
- 해당 정보만을 기반으로 대책을 세우고 시도하더라도,
  - 개선 폭이 상당히 **국소적**이며, 효과도 제한적인 경우가 많음
- `점`과 `점`을 이어 `선`을 만들어가며, **전체적인 흐름**을 파악할 수 있어야 함
- 예시
  - **페이지 이탈률**를 확인하는 방법은 `점`
  - 입력 양식에서 `입력, 확인, 완료`에 흐르는 일련의 흐름에서 이탈률 계산 -> `선`
  - 이러한 정보를 **입력 양식**을 개선하는 팀과, 검색기능을 개선하는 팀에게 줄경우
    - 문제를 빠르게 찾고, 개선이 가능함

### 조감도를 사용해 사용자 행동 시각화 하기
- 어떤 보고서로 **상황**을 파악하면 좋을지
- 일반적으로 **서비스 전체**를 파악할 수 있는 **조감도**를 만드는 것이 좋음
- 조감도를 작성하면
  - **사이트**와 **조직**의 **전체적인 이미지**를 파악할 수 있으며
  - `어디`에 문제가 있는지,
  - 어떤 부분을 개선할 지
  - 팀을 개섢날 수 있는지 파악이 가능

### 단순 업무시의 문제 상황
- 사용자가 **최상위 페이지**를 기반으로 접근할 것으로 생각해서, **최상위 페이지**를 최적화 했으나
  - 실제 사용자는 `구글 검색`등을 통해 **상세 화면**접근 하는 경우가 많아
  - 최적화에 의미가 없었던 상황
- **입력 양식** 최적화에 주력하였으나,
  - 알고보니, 사람들이 **입력 양식**까지 가는 것이 힘들었던 경우라, 최적화에 의미가 없음

### 서비스 형태에 맞게 조감도 작성하기
- **서비스 전체**를 파악하는 조감도는 **웹사이트 이외**의 경우에도 활용할 수 있음
- 다양한 서비스를 포함하는 **플랫폼**도
  - 서비스 이용 상황과 매출처럼, `개별적인 서비스 상황을 파악`할 때 활용할 수 있음 
  - 이를 활용하면 각각의 서비스를 담당하는 팀에게 정보를 전달해 서비스를 최적화 할 수 있음
- 어떻게 조감도를 표현할지는 **서비스**마다 다르나
  - `누가 보아도 쉽고, 확실하게 이해할 수 있는 조감도`가 좋은 조감도

### 정리
- 데이터 추출만 분석 담당자의 역할이라고 생각하는 경우가 많지만,
  - 그러한 결과를 바탕으로 `서비스`와 `조직`을 개선하는 것도 **분석 담당자**의 역할
- 따라서 **조감도**를 사용해 서비스와 조직의 상태를 파악하고,
  - 멤버 전원의 **서비스의 상황 이해**를 도우며 `길`을 제시할 수 있어야 함
- 데이터 추출은 위 과정중 하나